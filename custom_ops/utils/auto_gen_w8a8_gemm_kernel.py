...
"""
gemm_template_case = """
void w8a8_gemm_M{M}_N{N}_TAILN{TAILN}_K{K}_B{BATCH}_P{PADDING}_{TYPE}(
    const int8_t * weight,
    const int8_t * input,
    {cutlass_type} * out,
...
void w8a8_gemm_M{M}_N{N}_TAILN{TAILN}_K{K}_B{BATCH}_P{PADDING}_{TYPE}(
    ...
    constexpr static int kCluster = 1;
    ...
    using Kernel_traits = Kernel_traits<
        ...
        M, TokenPackSize, kBlockN_TAIL, kCluster, int8_t,
        {cutlass_type}>;
    run_gemm<int8_t, {cutlass_type},
        Kernel_traits, M, K, Batch, TokenPackSize>
        (weight, input, out, weight_scale,
        input_scale, tokens, max_tokens, stream);
}}
"""
...
dtype = ["BF16", "FP32", "FP16"]  # Added FP32 as common output type for w8a8
...
template_head_file = open(f"{file_dir}w8a8_gemm_template.h", "w")
    ...
    for case in gemm_case:
        for n in n_range:
            ...
            template_cu_file = open(
                f"{file_dir}w8a8_gemm_M{case[0]}_N{n}_TAILN{0}_K{case[1]}_B{case[2]}_P{case[3]}_{type}.cu", "w"
            )
            ...
            template_cu_file = open(
                f"{file_dir}w8a8_gemm_M{case[0]}_N{256}_TAILN{n-16}_K{case[1]}_B{case[2]}_P{case[3]}_{type}.cu", "w"
            )
            template_cu_file.write(gemm_template_cu_head)
            template_cu_file.write(
                gemm_template_cu_template.format(
...
for type in dtype:
    ...
    for case in gemm_case:
        ...
        w8a8_gemm_M{M}_N{N}_TAILN{TAILN}_K{K}_B{BATCH}_P{PADDING}_{TYPE}(__VA_ARGS__);  \\""".format(
    ...
    template_head_file.write(
        """    } else {   \\
            PADDLE_THROW(phi::errors::Unimplemented("W8a8 not supported m=%d k=%d  batch=%d  token_padding_size=%d  kBlockN=%d  tailN=%d\\n", _M, _K, _BATCH, _TokenPaddingSize, _kBlockN, _TailN));    \\
        }     \\
    }"""
    )
